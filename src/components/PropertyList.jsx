...
import { FaHome, FaMapMarkerAlt, FaDollarSign, FaUsers, FaUser } from 'react-icons/fa';
...
const PropertyList = ({ properties, users }) => {
  ...
  return (
    <div className="flex flex-col w-full max-w-6xl min-h-[33rem] justify-between bg-white p-4 md:p-8 rounded-lg shadow-lg" ref={containerRef}>
        ...
        {currentItems.map(property => (
          ...
          >
            <div className="flex flex-col mb-4">
              <h3 className="text-2xl font-semibold text-green-700 mb-2">{property.title}</h3>
              <div className="flex items-center mb-3">
                <FaMapMarkerAlt className="text-green-600 text-lg mr-2" />
                <p className="text-gray-800 text-md">{property.location}</p>
              </div>
              <div className="flex items-center mb-3">
                <FaDollarSign className="text-green-600 text-lg mr-2" />
                <p className="text-gray-800 text-md">${property.price}</p>
              </div>
              <div className="flex items-center mb-4">
                <FaUsers className="text-green-600 text-lg mr-2" />
                <p className="text-gray-800 text-md">Interested Users: {property.interestedBy.length}</p>
              </div>
              <div className="flex flex-col space-y-2">
                {property.interestedBy.map(userId => {
                  const user = users.find(u => u._id === userId);
                  return (
                    <div key={userId} className="flex items-center space-x-2 text-gray-700">
                      <FaUser className="text-gray-600 text-sm" />
                      <p className="text-sm">{user ? user.name : 'Unknown User'}</p>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="flex justify-center mt-8">
        <ReactPaginate
          ...
          previousClassName={`flex items-center justify-center w-20 h-8 border border-gray-300 rounded-lg ${currentPage === 0 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700'}`}
          previousLinkClassName="flex items-center justify-center w-full h-full"
          nextClassName={`flex items-center justify-center w-20 h-8 border border-gray-300 rounded-lg ${currentPage === pageCount - 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700'}`}
